...
public class ArticleDaoImpl extends BaseDaoSupport<Article> implements ArticleDao {
		...
		db.query("SELECT article_id, title, permalink, content, tag, create_time, stat, "
				...
				new Object[] { type, status }, new int[] { Types.INTEGER,
					...
					public void processRow(ResultSet rs) throws SQLException {
						Article article = new Article(rs.getLong(1), rs.getString(2), rs
	...
	public int update(final Article t) {
		int r = db
				.update("UPDATE article SET title=?, permalink=?, content=?, tag=?, create_time=?, stat=?, hits=?, "
	...
	public int updateColumn(long id, String col, int type, Object value) {
		int r = db.update("UPDATE article SET " + col + "=? WHERE article_id=?",
				new Object[]{ value, id }, new int[]{ type, Types.BIGINT});
		log.info("Update article " + id + " column " + col);
		return r;
	}
