...
public class ItemDaoImpl extends BaseDaoSupport<Item> implements ItemDao {
	...
	public int update(final Item t) {
		int r = db.update(
				"UPDATE item SET item_name=?, permalink=?, description=?, count=?, item_order=?,"
	...
	public Item getItemById(long id) {
		return db.queryForObject(
				"SELECT item_id, item_name, permalink, description, count, item_order, item_type,"
				...
				new Object[] { id }, new int[] { Types.BIGINT }, new RowMapper<Item>() {
					...
					public Item mapRow(ResultSet rs, int rowNum) throws SQLException {
						return new Item(rs.getLong(1), rs.getString(2), rs.getString(3), rs
	...
	public Map<String, Item> getItemMap(byte type, String order) {
		...
		db.query("SELECT item_id, item_name, permalink, description, count, item_order, item_type,"
				...
				new Object[] { type }, new int[] { Types.INTEGER, Types.INTEGER },
	...
	public List<Item> getItemsByRelationship(long id) {
		...
		db.query(
				"SELECT i.item_id, item_name, permalink, description, count, item_order, item_type, "
						...
						+ "r.article_id=? AND stat>0", new Object[] { id },
				new int[] { Types.BIGINT }, new RowCallbackHandler() {
					@Override
					public void processRow(ResultSet rs) throws SQLException {
						Item item = new Item(rs.getLong(1), rs.getString(2), rs.getString(3), rs
								.getString(4), rs.getInt(5), rs.getByte(6), rs.getByte(7), rs
	...
	public List<Item> getItemsByRelationship(long id, byte type, String order) {
		...
		db.query(
				"SELECT i.item_id, item_name, permalink, description, count, item_order, item_type, "
						+ "parrent_id, stat FROM item i, relationship r WHERE i.item_id=r.item_id AND "
						+ "r.article_id=? AND item_type=? AND stat>0 ORDER BY " + order,
				new Object[] { id, type }, new int[] { Types.BIGINT, Types.INTEGER },
				new RowCallbackHandler() {
					@Override
					public void processRow(ResultSet rs) throws SQLException {
						Item item = new Item(rs.getLong(1), rs.getString(2), rs.getString(3), rs
