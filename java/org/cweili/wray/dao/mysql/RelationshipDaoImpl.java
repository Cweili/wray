...
public class RelationshipDaoImpl extends BaseDaoSupport<long[]> implements RelationshipDao {
	...
	public long save(long[] t) {
		return db.update("INSERT INTO relationship (article_id, item_id) VALUES (?,?)", t,
	...
	public int remove(long[] t) {
		return db.update("DELETE FROM relationship WHERE article_id=? AND item_id=?", t, new int[] {
	...
	public int[] saveOrUpdate(Object o, List<Long> relatedIds) {
		...
		if (o instanceof Article) {
			...
			return null;
		StringBuilder sql = new StringBuilder("DELETE FROM relationship WHERE ").append(notIN)
				.append(" NOT IN (");
