...
(function ($) {
	...
	$.wysiwyg.controls.image = {
		...
		init: function (Wysiwyg) {
			var self = this, adialog, formImageHtml, regexp, dialogReplacements, translation,
				img = {
					alt: "",
			...
			formImageHtml += '</div></div>' +
				...
				'<div class="form-row form-row-last"><label for="name"></label><div class="form-row-value"><input type="submit" class="submit" value="{submit}"/> ' +
				'<input type="reset" class="submit" value="{reset}"/></div></div></fieldset></form>';

			for (var key in dialogReplacements) {
				if ($.wysiwyg.i18n) {
